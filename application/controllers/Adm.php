...
class Adm extends CI_Controller
{
    ...
    public function add_stkitem()
    {
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $data_array = array(
                "catg"        => $this->input->post('catg'),
                "sub_catg_id"        => $this->input->post('subcatg'),
                "item_name"     =>  $this->input->post('item_name'),
                "created_by"    =>  $this->session->userdata('user_name'),
                "created_dt"    =>  date('Y-m-d H:i:s')
            );
    ...
    function get_subcatg_list()
    {
        $catg_id = $this->input->get('catg_id');
        $where = array('catg_id' => $catg_id);
        $data = $this->Master->f_get_particulars("md_stk_sub_catg", NULL, $where, 0);
    ...
    public function edit_stkitem()
    {
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $select = 'a.id, a.item_name, a.catg catg_id, b.category catg, a.sub_catg_id';
            $where   =  array('a.id' => $this->input->get('id'), 'a.catg=b.sl_no' => null);
            $table_name = 'md_stk_item a, md_catg b';
            $item = $this->Master->f_get_particulars($table_name, $select, $where, 1);
            $data['item']  =  $item;
            $data['subcatg_list']  =  $this->Master->f_get_particulars('md_stk_sub_catg', null, array('catg_id' => $item->catg_id), 0);
            $this->load->view('common/header');
    ...
    function itemsubcat()
    {
        $select = 'a.id, b.category, a.name';
        $where = array(
            'a.catg_id=b.sl_no' => null
        );
        $table_name = 'md_stk_sub_catg a, md_catg b';
        $data['itemsubcatg']  =  $this->Master->f_get_particulars($table_name, $select, $where, 0);
        $this->load->view('common/header');
        $this->load->view('itemsubcatg/view', $data);
        $this->load->view('common/footer');
    }
    ...
    function subcatgadd()
    {
        ...
        if ($id > 0) {
            $where = array('id' => $id);
            $subcatg_data = $this->Master->f_get_particulars("md_stk_sub_catg", NULL, $where, 1);
            $selected = array(
                'catg_id' => $subcatg_data->catg_id,
                'id' => $id,
                'name' => $subcatg_data->name
            );
        }
        $catg_list = $this->Master->f_get_particulars("md_catg", NULL, NULL, 0);
        $data['catg_list'] = $catg_list;
        $data['selected'] = $selected;
        $this->load->view('common/header');
        $this->load->view('itemsubcatg/edit', $data);
        $this->load->view('common/footer');
    }

    function save_subcatg()
    {
        $data = $this->input->post();
        if ($this->Master->save_subcatg($data)) {
            $this->session->set_flashdata('msg', 'Successfully Added!');
            redirect('adm/itemsubcat');
        } else {
            $this->session->set_flashdata('msg', 'Data Not Added!');
            redirect('adm/subcatgadd');
        }
    }

    function delete_subcatg()
    {
        $where = array(
            'id' => $this->input->get('id')
        );
        $this->Master->f_delete('md_stk_sub_catg', $where);
        //For notification storing message
        $this->session->set_flashdata('msg', 'Successfully deleted!');
        redirect('adm/itemsubcat');
    }
    ...
    /////////////////////////// STAKEHOLDER TYPE MASTER //////////////////////////////
    function stakeholder_type()
    {
        $data['stkh_list']  =  $this->Master->f_get_particulars("md_stakeholder_type", NULL, NULL, 0);
        $this->load->view('common/header');
        $this->load->view('stakeholder_type/view', $data);
        $this->load->view('common/footer');
    }

    function add_stakeholder_type()
    {
        $id = $this->input->get('id');
        $selected = array(
            'id' => '',
        ...
        if ($id > 0) {
            $where = array('id' => $id);
            $stkhld_type_data = $this->Master->f_get_particulars("md_stakeholder_type", NULL, $where, 1);
            $selected = array(
                'id' => $id,
                'name' => $stkhld_type_data->name
            );
        }
        $data['selected'] = $selected;
        $this->load->view('common/header');
        $this->load->view('stakeholder_type/entry', $data);
        $this->load->view('common/footer');
    }

    function save_stakeholder_type()
    {
        $data = $this->input->post();
        if ($this->Master->save_stakeholder_type($data)) {
            $this->session->set_flashdata('msg', 'Successfully Added!');
            redirect('adm/stakeholder_type');
        } else {
            $this->session->set_flashdata('msg', 'Data Not Added!');
            redirect('adm/add_stakeholder_type');
        }
    }

    function delete_stakeholder_type()
    {
        $where = array(
            'id' => $this->input->get('id')
        );
        $this->Master->f_delete('md_stakeholder_type', $where);
        //For notification storing message
        $this->session->set_flashdata('msg', 'Successfully deleted!');
        redirect('adm/stakeholder_type');
    }
    ...
    function testFun()
    {
        $data = [
            ...
            'Refill',
            'Rubber Band',
            'Rent Collection Recpt.',
            'Register (Training Related Fund)',
            'Register (Meeting/ Workshop related Fund)',
            'Register ( Hall Booking)',
            'Register (Dormitary Booking)',
            'Register (Canteen)',
            'Register ( Dead Stock & Bedding Materials)',
            'Register (Room Booking)',
            'Stapler (Big)',
            'Stapler (Small)',
            'Stapler Pin (Big)',
            'Stapler Pin (Small)',
